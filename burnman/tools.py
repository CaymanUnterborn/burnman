...
def Stoich(EM,CMF, UMF, PctPv, PctFePv,PctFePer,PctOl,PctFo,PctFerro):

    #Calculate molar percentages of the other minerals present in the mantle
    Per = 1. - PctPv			# Percent Periclase

    PctFay = 1. - PctFo		# Percent Faylite

    PctOrthoclase = 1.-PctOl     #Percent Orthoclase
    #molar mass of each element in g/mol
    Si = 28.085				# Silicon molar mass
    O = 15.999				# Oxygen mass
    Mg = 24.305				# Magnesium mass
    Fe = 55.845				# Iron mass
    inGrams = 5.97*(10.**27.)	# value to convert earth masses  into grams

    #Calculate molar weight of each component mineral
    PvMol = ( Mg * ( 1. - PctFePv )) + ( Fe * PctFePv ) + Si + ( 3. * O )		# Molar Mass of Perovskite in Mantle
    PerMol = ( Mg * ( 1. - PctFePer )) + ( Fe * PctFePer ) + O				# Molar Mass of Periclase in Mantle
    OlMol = (2. * (( Mg * PctFo ) + ( Fe * PctFay )) )+ Si + ( 4. * O)			# Molar Mass of Olivine in Upper Mantle
    OrthoclaseMol = (( Mg * PctOrthoclase ) + ( Fe * PctFerro))+Si+ (3.*O)

    #convert masses to grams
    UpperMantleMass = ( UMF * EM ) * inGrams
    CoreMass = ( CMF  * EM ) * inGrams	# Core Mass in grams
    LowerMantleMass = ( EM * inGrams ) - ( CoreMass + UpperMantleMass )	# Lower Mantle Mass in grams

    #Calculate the number of moles for each layer
    LowerMantleMole = LowerMantleMass / (( PctPv * PvMol ) + ( Per * PerMol ))	# Number of moles of Mantle
    UpperMantleMole = UpperMantleMass / ((PctOl*OlMol) + (PctOrthoclase*OrthoclaseMol))	# Number of moles of Upper Mantle
    CoreMole = CoreMass / Fe

    #Calculate Moles Fe in mantle
    MolesFe_LowerMantle = ((PctPv * PctFePv ) + ( Per * PctFePer ))*LowerMantleMole			# Percent of Mantle that is Fe
    MolesFe_UpperMantle = ((2.*PctOl*PctFay)+(PctOrthoclase*PctFerro))*UpperMantleMole

    MolesMg_LowerMantle = ((PctPv * (1.-PctFePv )) + ( Per * (1.-PctFePer) ))*LowerMantleMole
    MolesMg_UpperMantle = ((2.*PctOl*(1.-PctFay))+(PctOrthoclase*(1.-PctFerro)))*UpperMantleMole

    MolesSi_LowerMantle = PctPv *LowerMantleMole
    MolesSi_UpperMantle = UpperMantleMole

    #Count it up
    TotMg = MolesMg_LowerMantle+MolesMg_UpperMantle	# Total number of Mg moles in the planet
    TotFe =  CoreMole + MolesFe_LowerMantle+MolesFe_UpperMantle 	# Total number of Fe moles in the planet
    TotSi = MolesSi_LowerMantle + MolesSi_UpperMantle

    SiFe = TotSi / TotFe	# Silicon to Iron ratio
    MgFe = TotMg / TotFe	# Magnesium to Iron ratio
    MgSi = TotMg / TotSi    #Magnesium to Iron ratio

    print("Molar Ratios for Planet")
    print("Si/Fe = ",'%.2f' % SiFe)
    print()
    print("Mg/Si = ",'%.2f' % MgSi)
    print()
